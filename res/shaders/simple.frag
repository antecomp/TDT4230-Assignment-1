...
vec3 ambientIntensity = vec3(0.05, 0.05, 0.05);
vec3 diffuseColour = vec3(0.1, 0.1, 0.1); // SHould be a uniform later :)
...
vec3 toBall = fragWSPosition - u_ballPosition;
...
void main()
{
    ...
    for(int i = 0; i < NUM_LIGHT_SOURCES; i++) {
        ...
        // // Shadow Check
        //  vec3 toLight = fragWSPosition - lightPositions[i];
        ...
        //     && length(toLight) <= length(toBall)
        //     && (dot(normalize(toBall), normalize(toLight)) > 0.01) // Looser threshold
        // ) {
        //     break;
        // }
